...
const ProductDetails = () => {
  ...
  const fetchProductDetails = async()=>{
    ...
    setActiveImage(dataReponse?.data?.productImage[0])
      ...
      const originalPrice = parseFloat(dataReponse?.data?.price);
      const sellingPrice = parseFloat(dataReponse?.data?.sellingPrice);
      if (originalPrice && sellingPrice) {
        const discountPercent = ((originalPrice - sellingPrice) / originalPrice) * 100;
        setDiscountPercentage(discountPercent.toFixed(0)); // Round to 0 decimal places
      } else {
          ...
          <div className='h-96 flex flex-col lg:flex-row-reverse gap-4'>
           ...
           {/***product details */}
           {
            ...
            (
              ...
              {/* Existing JSX */}
             <div className='border-2 border-blue-600 rounded px-3 py-1 min-w-[120px] font-medium text-white bg-green-600 hover:text-blue-600 hover:bg-white'>Discount: {discountPercentage}%</div>
             {/* Existing JSX */}
            </div>
